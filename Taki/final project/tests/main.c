//WRITTEN (proudly) BY: Ido Hirschmann 211329883
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
...
//func declaration
void printWelcome();//prints welcome
PLAYER * getPlayerData(int numOfPlayers, STATISTICS stats[]);//gets the number of players and player names
void setPlayerData(PLAYER * newPlayer, int ind, STATISTICS stats[]);//set the player array and data
void drawFirstCard(int* ptype, int* pcolor);//drew the first card of the game
void drawCard(int* ptype, int* pcolor,STATISTICS stats[]);//drew a random type and color for onr card
void printCard(int type, int color);// prints a single card
void printPlayersCards(PLAYER *players, int playerTurn);//prints a single player cards array
CARD * myRealloc(CARD * playersCard ,int * pPsize, int logSize);// creates a new CARD array for a player in case that he need to drew card while his card array is full
void setStatistic(STATISTICS arr[]);//set the base stats array- type.
int colorCard();//gets the desired color from the user.
CARD *removeCardFromPlayer(CARD * playersHand, int cardToRemove, int *pLogSize, int phySize);//remove card from players cards array
bool roundOfPlay(PLAYER players[],int size ,int i, CARD* upperCard, int * pplayersChoice,STATISTICS stats[]);//a single round of the game
CARD *addCardToPlayerHand(CARD * playersHand, int *pLogSize, int *pphySize,STATISTICS stats[]);//add card to players card array
bool isChoiceValid(CARD* playersHand, int playersChoice, CARD upperCard, int logSize, int phySize);//checks if players choice of card is valid
